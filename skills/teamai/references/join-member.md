...
- **`git.woa.com/...`** (Tencent TGit / 工蜂) → `gf auth login` (approve in the
  browser / iOA). `teamai init` auto-installs the `gf` CLI if it is missing; no
  `GITLAB_URL` is needed. Headless alternative: set `TGIT_TOKEN`.
